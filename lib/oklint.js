...
// (whitelist), reads the .oklint (yaml) file and calls the modules
  ...
  try {
    ...
    for (const moduleName in config) {
      const moduleConfig = config[moduleName],
            modulePath = './lint_modules/'+moduleName.replace(/-/g, '_'),
            moduleFn = require(modulePath)
      console.log(`Applying module ${moduleName}...`)
      moduleFn(moduleConfig)(nodepath.dirname(path), filter)
    }
  } catch (e) {
    die(e)
  }
...
if (file) {
  ...
  // file and pass the file as filter (array) to the modules
